...
class Wall:
    ...
    def draw(self):
        ...
        if self.orientation == 'w':
            pygame.draw.line(self.screen, self.color, (self.x - self.l // 2, self.y - self.l // 2),
                             (self.x + self.l // 2, self.y - self.l // 2), width=self.l * 3 // 20)
        '''
        Стена снизу:
        '''
...
class Key:
    ...
    def draw(self):
        pygame.draw.rect(self.screen, (230, 230, 230),
                         (self.x - self.l // 20 * 9, self.y - self.l // 20 * 9, self.l // 10 * 9, self.l // 10 * 9))
        pygame.draw.line(self.screen, (210, 210, 0), (self.x, self.y + self.l * 2 // 5),
                         (self.x, self.y - self.l * 2 // 5), width=self.l // 10)
...
class Minotaur:
    ...
    def draw(self):
        pygame.draw.rect(self.screen, (230, 230, 230),
                         (self.x - self.l // 20 * 9, self.y - self.l // 20 * 9, self.l // 10 * 9, self.l // 10 * 9))
        pygame.draw.polygon(self.screen, (120, 50, 0),
                            [(self.x, self.y + self.l * 2 // 5), (self.x - self.l * 3 // 10, self.y - self.l // 20 * 7),
                             (self.x + self.l // 20 * 6, self.y - self.l // 20 * 7)])
        pygame.draw.polygon(self.screen, (0, 0, 0), [(self.x - self.l // 20 * 5, self.y - self.l // 20 * 7),
                                                     (self.x - self.l // 20 * 4, self.y - self.l // 20 * 9),
                                                     (self.x - self.l // 20 * 4, self.y - self.l // 20 * 7)])
        pygame.draw.polygon(self.screen, (0, 0, 0), [(self.x + self.l // 20 * 5, self.y - self.l // 20 * 7),
                                                     (self.x + self.l // 20 * 4, self.y - self.l // 20 * 9),
                                                     (self.x + self.l // 20 * 4, self.y - self.l // 20 * 7)])
        pygame.draw.circle(self.screen, (255, 0, 0), (self.x - self.l // 20 * 2, self.y - self.l // 20 * 2),
                           self.l // 20 * 1)
        pygame.draw.circle(self.screen, (255, 0, 0), (self.x + self.l // 20 * 2, self.y - self.l // 20 * 2),
                           self.l // 20 * 1)
...
class Dead_minotaur:
    ...
    def draw(self):
        pygame.draw.rect(self.screen, (230, 230, 230),
                         (self.x - self.l // 20 * 9, self.y - self.l // 20 * 9, self.l // 10 * 9, self.l // 10 * 9))
        pygame.draw.polygon(self.screen, (120, 50, 0), [(self.x - self.l // 20 * 9, self.y + self.l // 20 * 9),
                                                        (self.x + self.l // 20 * 9, self.y + self.l // 20 * 9),
                                                        (self.x + self.l // 20 * 2, self.y)])
        pygame.draw.polygon(self.screen, (0, 0, 0), [(self.x + self.l // 20 * 4, self.y + self.l // 20),
                                                     (self.x + self.l // 20 * 6, self.y + self.l // 20),
                                                     (self.x + self.l // 20 * 5, self.y + self.l // 20 * 2)])
        pygame.draw.polygon(self.screen, (0, 0, 0), [(self.x + self.l // 20 * 7, self.y + self.l // 20 * 7),
                                                     (self.x + self.l // 20 * 8, self.y + self.l // 20 * 8),
        ...
        pygame.draw.circle(self.screen, (0, 0, 0), (self.x + self.l // 20 * 3, self.y + self.l // 20 * 7),
                           self.l // 20 * 2)
...
class Revival:
    ...
    def draw(self):
        pygame.draw.rect(self.screen, (230, 230, 230),
                         (self.x - self.l // 20 * 9, self.y - self.l // 20 * 9, self.l // 10 * 9, self.l // 10 * 9))
        pygame.draw.line(self.screen, (255, 0, 0), (self.x, self.y - self.l // 20 * 8),
                         (self.x, self.y + self.l // 20 * 8), width=self.l // 20 * 8)
        pygame.draw.line(self.screen, (255, 0, 0), (self.x - self.l // 20 * 8, self.y),
                         (self.x + self.l // 20 * 8, self.y), width=self.l // 20 * 8)
...
class Player:
    ...
    def draw(self):
        pygame.draw.rect(self.screen, (0, 200, 0),
                         (self.x - self.l // 20 * 8, self.y - self.l // 20 * 8, self.l // 20 * 16, self.l // 20 * 16),
                         width=self.l // 10)
...
class Armory:
    ...
    def draw(self):
        pygame.draw.rect(self.screen, (230, 230, 230),
                         (self.x - self.l // 20 * 9, self.y - self.l // 20 * 9, self.l // 10 * 9, self.l // 10 * 9))
        pygame.draw.line(self.screen, (120, 50, 0), (self.x - self.l // 20 * 9, self.y + self.l // 20 * 9),
                         (self.x - self.l // 20 * 5, self.y + self.l // 20 * 5), width=self.l // 20 * 3)
        pygame.draw.polygon(self.screen, (160, 160, 160), [(self.x - self.l // 20 * 8, self.y + self.l // 20 * 3),
                                                           (self.x - self.l // 20 * 3, self.y + self.l // 20 * 8),
                                                           (self.x + self.l // 20 * 8, self.y - self.l // 20 * 8)])
...
class Arrow_botton:
    ...
    def draw(self):
        ...
        pygame.draw.rect(self.screen, (100, 150, 200),
                         (self.x + 210 / 480 * self.width, 7 / 60 * self.height, 1 / 30 * self.width,
                          1 / 20 * self.height))
...
class Opened_door:
    ...
    def draw(self):
        ...
        pygame.draw.line(self.screen, (155, 0, 0), (self.x, self.y + 2 * self.l),
                         (self.x + self.l // 10, self.y + 2 * self.l), width=self.l // 20)
        pygame.draw.polygon(self.screen, (155, 0, 0),
                            [(self.x + self.l, self.y), (self.x + self.l // 10, self.y + self.l // 3),
                             (self.x + self.l // 10, self.y + self.l * 2.33), (self.x + self.l, self.y + 2 * self.l)])
...
class Typing_window:
    ...
    def draw(self):
        pygame.draw.rect(self.screen, (200, 200, 200), (self.x - 200, self.y - 100, 400, 200))
        pygame.draw.rect(self.screen, (0, 0, 0), (self.x - 200, self.y - 100, 400, 200), width=10)
        pygame.draw.line(self.screen, (100, 100, 100), (self.x - 190, self.y + 90), (self.x + 190, self.y + 90),
                         width=3)
        f1 = pygame.font.Font(None, 80)
...
class Closed_door:
    ...
    def draw(self):
        ...
        pygame.draw.line(self.screen, (155, 0, 0), (self.x, self.y), (self.x, self.y + 2 * self.l), width=self.l // 20)
        pygame.draw.line(self.screen, (155, 0, 0), (self.x, self.y + 2 * self.l),
                         (self.x + self.l, self.y + 2 * self.l), width=self.l // 20)
...
class Fully_opened_door:
    ...
    def draw(self):
        ...
        pygame.draw.line(self.screen, (155, 0, 0), (self.x, self.y + 2 * self.l),
                         (self.x + self.l, self.y + 2 * self.l), width=self.l // 20)
        pygame.draw.polygon(self.screen, (155, 0, 0),
                            [(self.x + self.l, self.y), (self.x + self.l // 10 * 19, self.y + self.l // 3),
                             (self.x + self.l // 10 * 19, self.y + self.l * 2.33),
                             (self.x + self.l, self.y + 2 * self.l)])
...
class Corner:
    ...
    def draw(self):
        pygame.draw.polygon(self.screen, (200, 200, 200),
                            [(self.width // 2, self.height // 5), (self.width, self.height // 5 * 2),
                             (self.width, self.height), (self.width // 2, self.height // 5 * 4)])
        pygame.draw.polygon(self.screen, (200, 200, 200),
                            [(self.width // 2, self.height // 5), (0, self.height // 5 * 2), (0, self.height),
                             (self.width // 2, self.height // 5 * 4)])
        pygame.draw.line(self.screen, (0, 0, 0), (self.width // 2, self.height // 5),
                         (self.width // 2, self.height // 5 * 4), width=10)
        pygame.draw.line(self.screen, (0, 0, 0), (self.width // 2, self.height // 5),
                         (self.width, self.height // 5 * 2), width=10)
        pygame.draw.line(self.screen, (0, 0, 0), (self.width // 2, self.height // 5), (0, self.height // 5 * 2),
                         width=10)
        pygame.draw.line(self.screen, (0, 0, 0), (self.width // 2, self.height // 5 * 4), (self.width, self.height),
                         width=10)
...
class Skull:
    ...
    def draw(self):
        ...
        pygame.draw.rect(self.screen, (0, 0, 0),
                         (self.x - self.l // 4, self.y + self.l // 4, self.l // 2, self.l // 10))
